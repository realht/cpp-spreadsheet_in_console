...
public:
    ...
    virtual double Evaluate(ExecuteFromSheet args) const = 0;
...
public:
    ...
    double Evaluate(ExecuteFromSheet args) const override {
        double result = 0;
        if (type_ == Type::Add) {
...
public:
    explicit CellExpr(const Position* cell)
        : cell_(cell) {
        //std::cout << " cell = " << cell << ", " << (*cell_).col << "," << (*cell_).row << ", " << cell->ToString() << '\n';
    }
    ...
    double Evaluate(ExecuteFromSheet args) const override {
        //std::cout << "in cellexpr col row " << cell_ << ", " << cell_->ToString() << ", " << (*cell_).col << "," << (*cell_).row << ", res = " << args(*cell_) << '\n';
        return args(*cell_);
    }
...
double FormulaAST::Execute(ExecuteFromSheet args) const {
